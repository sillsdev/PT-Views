...
.err-para-p-pre-not-b-ml1-c-tr::after {content:'The preceding is not \\b or \\ml1 or \\c or \\tr #62';}
.err-para-p-pre-not-b-ml1-c-tr {border-top:2pt solid red}
...
.err-para-sla-pre-not-sl1-pvr::after {content:'The preceding is not \\sl1 or \\pvr #63';}
.err-para-sla-pre-not-sl1-pvr {border-top:2pt solid red}
...
.err-note-f-post-not-ros {}
      ...
      <xsl:element name="span">
         <xsl:attribute name="class">
            ...
            <xsl:if test="substring(node()[last()],string-length(text()),1) = ' '">
               <xsl:if test="count(following-sibling::node()) &gt; 0 and @style != 'fr'">
   ...
   <xsl:template match="para[@style = 'p']">
      <xsl:element name="div">
         <xsl:attribute name="class">
            <xsl:value-of select="@style"/>
            <!--ck-id 62 - rank=5-->
            <xsl:if test="not(preceding-sibling::*[1][@style = 'rem' or @style = 'b' or @style = 'ml1' or name() = 'chapter' or name() = 'table'])">
               <xsl:text> err-para-p-pre-not-b-ml1-c-tr</xsl:text>
            </xsl:if>
            <!--ck-id 39 - rank=8-->
   ...
   <xsl:template match="para[@style = 'sla']">
      <xsl:element name="div">
         <xsl:attribute name="class">
            ...
            <!--ck-id 63 - rank=5-->
            <xsl:if test="not(preceding-sibling::*[1][@style = 'rem' or @style = 'sl1' or @style = 'pvr'])">
               <xsl:text> err-para-sla-pre-not-sl1-pvr</xsl:text>
            </xsl:if>
            <!--ck-id 43 - rank=0-->
            <xsl:if test="string-length(text()) = 0 and not(*)">
               <xsl:if test="not(@style = 'b' or @style = 'b2' or @style = 'ntn' or @style = 'mt9' or @style = 'rem')">
   ...
   <xsl:template match="table[@style = '']">
      <xsl:element name="div">
         <xsl:attribute name="class">table<!--ck-id 64 - rank=5--><xsl:if test="not(preceding-sibling::*[1][@style = 'rem' or @style = 'b' or @style = 'b3'])">
