...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="para[@style='sl1']">
        ...
        <xsl:variable name="post-style-5" select="following-sibling::*[3]/@style"/>
        <xsl:variable name="post-style-6" select="following-sibling::*[4]/@style"/>
        <xsl:text> </xsl:text>
        <span class="verse">
            <xsl:apply-templates select="text()[1]" mode="verse"/>
        </span>
        <xsl:choose>
            <xsl:when test="$post-style-2 = 'ml1' and $post-style-6 = 'ml1' and $post-style-5 = 'mlor' ">
                <xsl:apply-templates select="following-sibling::*[6]" mode="only"/>
            </xsl:when>
            <xsl:when test="$post-style-1 = 'ml1' and $post-style-5 = 'ml1' and $post-style-4 = 'mlor' ">
                <xsl:apply-templates select="following-sibling::*[5]" mode="only"/>
            </xsl:when>
            <xsl:otherwise>
                <span class="{@style}">
                    <xsl:text>((...))</xsl:text>
