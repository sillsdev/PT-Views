...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gen="dummy-namespace-for-the-generated-xslt" xmlns:f="myfunctions" exclude-result-prefixes="f">
    ...
    <xsl:variable name="xml-pre" select="concat($projectpath,'\output\',$target,'-struc-data.xml')"/>
    ...
    <xsl:variable name="g1">
        <xsl:for-each-group select="/*/*[string-length(disable) = 0 and child::grp = $target]" group-by="child::element">
    ...
    <xsl:template name="gen-static">
        ...
        <gen:template match="chapter[@number]">
            <gen:element name="h4">
                <gen:text>Version: <xsl:value-of select="$target"/>&#160;</gen:text>
                <gen:value-of select="$version"/>
            </gen:element>
